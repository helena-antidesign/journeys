...
var openCheckin = function(){
  $('.JS__hotel-checkin').toggleClass('open');
}

var closeCheckin = function(){
  $('.JS__hotel-checkin').removeClass('open');
}


$('.JS__hotel-checkin').click( function(event) {
  event.stopPropagation();
  openCheckin();
});

$(document).click( function(event){
    if ( !$(event.target).closest('.JS__hotel-checkin').length ) {
      closeCheckin();   
    }
});
